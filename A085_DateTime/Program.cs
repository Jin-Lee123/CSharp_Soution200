...
namespace A085_DateTime
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("{0}과 {1}의 차이는 {2}일입니다", date1.ToString("yyyy년 M월 d일"), date2.ToString("yyyy년 M월 d일"), date1.Subtract(date2).Days);
            Console.WriteLine("\n오늘 : {0}", DateTime.Today);

            DateTime y = DateTime.Today.AddDays(-1);  //어제
            Console.WriteLine("어제  : {0}", y.ToShortDateString());

            DateTime t = DateTime.Today.AddDays(1);  //오늘
